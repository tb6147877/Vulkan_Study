...
//
//	uint32_t mipLevels;//自己创建图片的mipmap
...
//
//	VkSampleCountFlagBits msaaSamples = VK_SAMPLE_COUNT_1_BIT;//GPU支持的采样数量
//
//	//用于缓存MSAA的buffer
//	VkImage colorImage;
...
//
//	//创建一张图片
//	void createImage(uint32_t width, uint32_t height, uint32_t mipLvs, VkSampleCountFlagBits numSmps, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory);
//
//	//这是一个helper函数，使image处于正确的layout，这个函数用于处理layout转变
//	void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLvs);
//
...
//	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLvs);
//
...
//
//	//创建mipmap
//	void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLvs);
//
//	//选择硬件支持的抗锯齿采样数量
//	VkSampleCountFlagBits getMaxUsableSampleCount();
//
//	//创建MSAA用的color buffer
