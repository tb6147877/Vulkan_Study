...
void Mipmap::pickPhysicalDevice() {
...
void Mipmap::createLogicalDevice() {
...
void Mipmap::createSurface() {
	if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
		throw std::runtime_error("failed to create window surface!");
	}
}

bool Mipmap::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
SwapChainSupportDetails Mipmap::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR Mipmap::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR Mipmap::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
VkExtent2D Mipmap::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
...
void Mipmap::createDescriptorSetLayout() {
...
void Mipmap::createGraphicsPipeline() {
...
VkShaderModule Mipmap::createShaderModule(const std::vector<char>& code) {
...
void Mipmap::createFramebuffers() {
...
void Mipmap::createCommandBuffers() {
...
void Mipmap::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
...
VkCommandBuffer Mipmap::beginSingleTimeCommands() {
...
void Mipmap::endSingleTimeCommands(VkCommandBuffer cmdbuf) {
...
void Mipmap::updateUniformBuffer(uint32_t currentImage) {
...
void Mipmap::createVertexBuffer() {
...
void Mipmap::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
void Mipmap::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void Mipmap::createUniformBuffer() {
...
void Mipmap::createTextureImage() {
	...
	mipLevels =static_cast<uint32_t>(std::floor(std::log2(std::max(texWidth, texHeight)))) + 1;//算出最多能分出几级mipmap
	...
	//为了生成mipmap，要添加VK_IMAGE_USAGE_TRANSFER_SRC_BIT，因为此时图片也变成了拷贝源
	...
	//由于vulkan只能处理整张图的VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL，所以我们把这部分挪到generateMipmaps()函数中
...
//mipmap一般都是预先生成好的，运行时直接用就好了
//TODO:编写mipmap生成器
void Mipmap::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLvs) {
	...
	VkCommandBuffer temp_cmd_buf = beginSingleTimeCommands();
	...
	for (uint32_t i = 1; i < mipLvs; i++) {
		...
		vkCmdPipelineBarrier(temp_cmd_buf,
	...
	barrier.subresourceRange.baseMipLevel = mipLvs - 1;//现在到了最后一级mipmap
	barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
	barrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
	barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
	...
	endSingleTimeCommands(temp_cmd_buf);
}


void Mipmap::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLvs) {
	VkCommandBuffer temp_cmdbuf = beginSingleTimeCommands();
	...
	barrier.subresourceRange.levelCount = mipLvs;
	barrier.subresourceRange.baseArrayLayer = 0;
	barrier.subresourceRange.layerCount = 1;
...
void Mipmap::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
	VkCommandBuffer temp_cmdbuf = beginSingleTimeCommands();
...
void Mipmap::createDepthResources() {
...
bool Mipmap::hasStencilComponent(VkFormat format) {
	return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}


VkFormat Mipmap::findDepthFormat() {
...
VkFormat Mipmap::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
void Mipmap::createTextureImageView() {
	textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

VkImageView Mipmap::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLvs) {
	//创建VkImageView的结构体
	...
	createInfo.subresourceRange.levelCount = mipLvs;
...
void Mipmap::createTextureSampler() {
	...
	samplerInfo.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;//与mipmap有关，sampler控制mipmap的采样
	samplerInfo.minLod = static_cast<float>(mipLevels/2); // 使mipmap的效果更清晰
...
void Mipmap::createImage(uint32_t width, uint32_t height, uint32_t mipLvs, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void Mipmap::createDescriptorPool() {
...
void Mipmap::createDescriptorSets() {
...
uint32_t Mipmap::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
bool Mipmap::isDeviceSuitable(VkPhysicalDevice device) {
...
QueueFamilyIndices Mipmap::findQueueFamilies(VkPhysicalDevice device) {
...
bool Mipmap::checkValidationLayerSupport() {
	uint32_t layerCount;
...
//得到Vulkan调试所需的所有扩展
std::vector<const char*> Mipmap::getRequiredExtensions() {
...
void Mipmap::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
	auto app = reinterpret_cast<Mipmap*>(glfwGetWindowUserPointer(window));//通过window获取已经绑定好的本类实体
	app->framebufferResized = true;
}

void Mipmap::setupDebugMessenger() {
...
void Mipmap::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
	createInfo = {};
...
std::vector<char> Mipmap::readFile(const std::string& filename) {
